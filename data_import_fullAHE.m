...
for i=3:num_of_p+2%3 as . and .. are not to be done
    ...
    for j=3:7%size(pathimg)
        ...
        %trainX(:,jtr)=reshape(adapthisteq(medfilt2(reshape(trainX(:,jtr),[240 320]),[3,3])),[imSize(1)*imSize(2) 1]);
        %trainX(:,jtr)=reshape(medfilt2(reshape(trainX(:,jtr),[240 320]),[3,3]),[imSize(1)*imSize(2) 1]);
        jtr=jtr+1;
        ...
        %testX(:,jte) = reshape(adapthisteq(medfilt2(reshape(testX(:,jte),[240 320]),[3,3])),[imSize(1)*imSize(2) 1]);
        %testX(:,jte) = reshape(medfilt2(reshape(testX(:,jte),[240 320]),[3,3]),[imSize(1)*imSize(2) 1]);
        jte=jte+1;
        if(jte>size(testX))
            break;
        end
